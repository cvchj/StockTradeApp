...
public class EntityContext {
  ...
  public String getRemark() {
      Api api = resource.getDomainType().getAnnotation(Api.class);
      if(api == null ){
          return getName();
      }
      return api.value();
  ...
  public PersistentEntity<?, ?> entity() {
    ...
    // 重写， 适配工程的spring data rest版本
    Optional<PersistentEntity<?, ?>> result = entities.getPersistentEntity(actualDomainType);
    return result.get();
  }
